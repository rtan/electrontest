    ...
    $("#tree2").fancytree({
        ...
        dnd: {
            ...
            initHelper: function (node, data) {
                ...
                $(".fancytree-active,.fancytree-selected", $("#tree2").$container)
...
//          data.dropMarker.toggleClass("fancytree-drop-copy", copyMode);
        ...
        edit: {
            ...
            close: function (event, data) {
                if (data.save && data.isNew) {
                    ...
                    $("#tree2").trigger("nodeCommand", {cmd: "addSibling"});
                }
            }
        },
        table: {
    ...
    $("#tree2").contextmenu({
        ...
        beforeOpen: function (event, ui) {
            ...
            $("#tree2").contextmenu("enableEntry", "paste", !!clipboard);
            node.setActive();
        },
    ...
    $("#toggleExpand2").on("click", () => {
        expandedAll = !expandedAll;
        $("#tree2").fancytree("getTree").visit(node => {
            node.setExpanded(expandedAll);
        });
    });

    $("#saveBtn2").on("click", () => {
        let tree = $("#tree2").fancytree("getTree");
        let d = tree.toDict(true);
        storage.set("tree2", JSON.stringify(d), error => {
        });
    });

    $("#loadBtn2").on("click", function () {
        storage.get("tree2", (error, data) => {
            $("#tree2").fancytree("option", "source", JSON.parse(data));
        });
    });
});